...
unsigned long vibretion_delay = 800; // a short delay after movment end to insure the system vibration end and radey for quiet recording <1s

// Function declarations
void moveStepper(int steps);
void updateMovement();
long calibrateStepsPerRevolution();
void check_for_handshake_msg();
...
void updateMovement() {
    ...
    if (isMoving && currentPosition < targetAngle) {
        if (currentPosition ==0){//on first move send recognition msg and start msg
            // let the recorder to record the first position
        ...
        if (millis() - previousTime >= delayTime) {
            ...
            // whait short time for wibaration to end
            customDelay(vibretion_delay);
...
void check_for_handshake_msg(){
    ...
    //if "recorder handshaked" is received then  react with "mapper handshaked"
    String handshake_msg = "Recorder handshaked";
    if(Serial.available() > 0){
        ...
        if(msg == handshake_msg){
            Serial.printf("Mapper handshaked\n");
            flag = 1;
        }
    }
}
